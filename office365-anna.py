...
def send_html_email(subject, html_content, from_email, password,
    ...
    try:
        # Create a socket – wrap it with SOCKS5 if a proxy is given
        if proxy:
...
def proxy_checker(proxies, valid_proxies, lock):
    """
    Worker that checks a list of proxies and adds the working ones
    to the shared `valid_proxies` list (thread‑safe via lock).
    """
...
def validate_combo(combo, smtp_servers, proxy_queue, lock):
    ...
    If a working proxy is available it will be taken from `proxy_queue`,
    used for the connection and – on success – put back into the queue.
    """
    ...
    for name, (smtp_host, smtp_port) in smtp_servers.items():
        proxy = None
        # Try to borrow a proxy from the queue
        with lock:
            if proxy_queue:
                proxy = proxy_queue.pop(0)

        try:
            # Create socket, optionally wrapped by proxy
            if proxy:
                proxy_ip, proxy_port = proxy.split(':')
                sock = socks.socksocket()
                sock.set_proxy(socks.SOCKS5, proxy_ip, int(proxy_port))
            ...
            with smtplib.SMTP(smtp_host, smtp_port, sock=sock) as server:
                ...
                # Proxy worked – put it back for reuse
                if proxy:
                    with lock:
        ...
        # If we used a proxy and the attempt failed, discard it
        if proxy:
            # Already removed from queue – do not re‑insert
            pass
...
def process_file(file_path, proxy_file_path=None, num_threads=10):
    ...
    valid_proxies = []
    proxy_queue = []                     # will hold working proxies for reuse
    if proxies:
    ...
    # --------------------------------------------------------------
    # 3. Validate combos using a thread pool
    # --------------------------------------------------------------
    lock = threading.Lock()
